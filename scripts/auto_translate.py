...
def translate_po_files(i18n_path, lang, label=None, only_num_untranslated=False, all_languages=False, dry_run=False):
            ...
            if file.endswith('.po'):
                ...
                for entry in po.untranslated_entries():
                    ...
                    try:
                        ...
                        # trans sometimes returns a string with a space before the ${, so we need to remove that, or our variables break
                        if "$ {" in comment_t:
    ...
    if len(errors) > 0:
        ...
        for error in errors:
            print(error)
        print ("Rerun this script with --only-num-untranslated to see how many entries were untranslated.")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-u", "--only-num-untranslated", action="store_true",
    ...
    if args.all:
        translate_po_files(i18n_euphorie, None,
                           label=args.label, only_num_untranslated=args.only_num_untranslated, all_languages=True, dry_run=args.dry_run)
    elif args.lang:
        translate_po_files(i18n_euphorie, args.lang,
                           label=args.label, only_num_untranslated=args.only_num_untranslated, dry_run=args.dry_run)
    else:
